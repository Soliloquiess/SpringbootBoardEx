...
//		System.out.println("ImageDAO.list()");
		...
		try {// 정상처리
			...
			String sql = " select i.no, i.title, i.id, m.name, i.writeDate, i.fileName "
					+ " from image i, member m " + search(pageObject,"i.")
...
//		System.out.println("ImageDAO.list().list - " + list);
	...
	// 2. 글보기
	public  ImageVO view(ImageVO inVo) throws Exception {
		// 콘솔에서 확인해야 할 내용.
		ImageVO vo = null;
		...
		try {
			...
			String sql = "select i.no, i.title, i.content, i.id, m.name, "
					...
					+ " i.fileName, "
					+ " (select 'LIKED' from image_like where no = ? and id = ? ) myLiked, "
					+ " (select count(*) from image_like where no = ?) likeCnt "
...
//			System.out.println("ImageDAO.view().sql - " + sql);
	...
	public int update(ImageVO vo)  throws Exception {
		...
		try {
			...
			con = DBInfo.getConnection();
			// 3. sql
			// 자신이 작성한 이미지 정보만 바꿀 수 있다. id = ?
			String sql = "update image set title = ?, content = ? where no = ? and id = ?";
			// 4. 실행객체 & 데이터
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, vo.getTitle());
			pstmt.setString(2, vo.getContent());
			...
			result = pstmt.executeUpdate();
			// 6. 결과표시 & 데이터 담기
			if(result == 0) throw new Exception("수정할 데이터가 존재하지 않거나 본인이 등록한 이미지가 아닙니다.");
			else System.out.println("이미지 게시판 정보 수정이 완료되었습니다.");
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
			throw new Exception("이미지 게시판 정보 수정 - 정보 수정 중 DB 오류 또는 본인의 글이 아닙니다.");
		} finally {
			// 7. 닫기
			DBInfo.close(con, pstmt);
		}
	...
	// 4-1. 이미지 정보 수정
	public int updatePhoto(ImageVO vo)  throws Exception {
		System.out.println("ImageDAO.updatePhoto().vo - " + vo);
		int result = 0;
		...
		try {
			...
			else System.out.println("이미지 정보가 수정되었습니다.");
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
			throw new Exception("이미지 게시판 이미지 수정 - 이미지 수정 중 DB 오류");
		} finally {
			// 7. 닫기
			DBInfo.close(con, pstmt);
		}
	...
	public int delete(ImageVO vo)  throws Exception {
		System.out.println("ImageDAO.delete().vo - " + vo);
		int result = 0;
		...
		try {
			...
			con = DBInfo.getConnection();
			// 3.
			String sql = "delete from image where no = ? and id = ?";
			// 4.
			pstmt = con.prepareStatement(sql);
			...
			// TODO: handle exception
			e.printStackTrace();
			throw new Exception("이미지 게시판 글삭제 - 글삭제 중 DB 오류 또는 본인이 작성한 글이 아닙니다.");
		} finally {
			// 7. 닫기
			DBInfo.close(con, pstmt);
		}
	...
	// 6. 좋아요 처리 - insert
	public int like(ImageVO vo)  throws Exception {
		System.out.println("ImageDAO.like().vo - " + vo);
		int result = 0;
		...
		try {
			...
			con = DBInfo.getConnection();
			// 3.
			String sql = "insert into image_like(no, id) values(?, ?)";
			// 4.
			pstmt = con.prepareStatement(sql);
			pstmt.setLong(1, vo.getNo());
			pstmt.setString(2, vo.getId());
			...
			// TODO: handle exception
			e.printStackTrace();
			throw new Exception("이미지 게시판 좋아요 - 좋아요 처리 중 DB 오류 또는 좋아요 처리가된 데이터입니다.");
		} finally {
			// 7. 닫기
			DBInfo.close(con, pstmt);
		}
	...
	// 7. 좋아요 취소 - delete
	public int likeCancel(ImageVO vo)  throws Exception {
		System.out.println("ImageDAO.likeCancel().vo - " + vo);
		int result = 0;
		...
		try {
			...
			con = DBInfo.getConnection();
			// 3.
			String sql = "delete from image_like where no = ? and id = ?";
			// 4.
			pstmt = con.prepareStatement(sql);
			pstmt.setLong(1, vo.getNo());
			pstmt.setString(2, vo.getId());
			...
			// TODO: handle exception
			e.printStackTrace();
			throw new Exception("이미지 게시판 좋아요 취소 - 좋아요 취소 중 DB 오류 또는 정보 확인하셔야 합니다.");
		} finally {
			// 7. 닫기
			DBInfo.close(con, pstmt);
		}
