...
public class QnaDAO {
	...
	public int increase(long no) throws Exception{
		...
		try { // 정상 처리
			...
			//5. 실행
			// update가 됐으면 1, 안됐으면 0 -> 번호에 따른 데이터가 없는 경우
			result = pstmt.executeUpdate();
			
			// 6. 데이터 표시
			System.out.println("QnaDAO.increase() - 조회수 1 증가되었습니다.");
	...
	public int update(QnaVO vo) throws Exception{
		...
		try { // 정상 처리
			...
			// 6. 데이터 표시
			System.out.println("QnaDAO.update() - 질문이나 답변 수정되었습니다.");
	...
	public int increaseOrdNo(QnaVO vo) throws Exception{
		...
		try { // 정상 처리
			...
			// 6. 데이터 표시
			System.out.println("QnaDAO.increaseOrdNo() - 순서번호 1 증가되었습니다.");
			System.out.println("QnaDAO.increaseOrdNo(). resault - " + result);
