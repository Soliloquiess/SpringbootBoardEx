...
public class SecurityConfig extends WebSecurityConfigurerAdapter{
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.formLogin()
		.loginPage("/login")
				.usernameParameter("email")		//여기엔 id를 넘겨줄 파라미터를 이름을 뭐라 할 건지(username=id)
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		...
		String query1 = "SELECT email ,password, enabled FROM member WHERE email = ?";
		String query2 ="SELECT email, auth FROM member  WHERE email = ?";
