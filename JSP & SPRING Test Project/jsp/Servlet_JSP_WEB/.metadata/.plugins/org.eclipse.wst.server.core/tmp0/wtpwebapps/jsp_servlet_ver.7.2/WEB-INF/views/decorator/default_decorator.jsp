...
// 		alert(window.performance.navigation.type);
	...
	$(document).ready(function() {
		...
		<c:if test="${!empty login}">
			...
			// 선택한 것에 데이터를 올려 놓는다. 태그 사이의 데이터를 바꾼다.(이게 load)
			$("#newMsgCnt").load("/ajax/msgCount.do");
			//newMsgCnt는 태그. 태그 안에 글자 바꾸는게 load. 선택한게$. 태그 사이의 데이터 바꿈
				...
				<div class="collapse navbar-collapse" id="myNavbar">
					...
					<ul class="nav navbar-nav navbar-right">
						<c:if test="${empty login }">
					    	...
					    	<li><a href="/member/write.do"><span class="glyphicon glyphicon-user"></span> 회원가입</a></li>
					    </c:if>
						<c:if test="${!empty login }">
