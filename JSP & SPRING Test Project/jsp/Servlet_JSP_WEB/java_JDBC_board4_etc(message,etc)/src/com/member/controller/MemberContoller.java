...
import com.member.service.MemberListService;
import com.member.service.MemberWriteService;
import com.member.vo.Login;
...
//				if(!isLooop) return;
	...
	boolean logined() throws Exception{
		...
		case "6":
			...
			ExecuteService.execute(new MemberListService(), null);
			break;
...
//			Login.setLogin(outVO);
		...
		case "2":
			...
			// 데이터 수집 - 아이디, 비밀번호, 이름 , 성별(남자,여자), 생년월일, 연락처, 이메일 ,사진
			ExecuteService.execute(new MemberWriteService(), InVO());
			break;
		case "3":
	...
	private MemberVO InVO() {
		MemberVO vo = new MemberVO();
		
		// 아이디, 비밀번호, 이름 , 성별(남자,여자), 생년월일, 연락처, 이메일 ,사진
		vo.setId(In.getString("아이디(중복이 되면 안된다.)"));
		vo.setPw(In.getString("비밀번호"));
		vo.setName(In.getString("이름"));
		vo.setGender(In.getString("성별(남자,여자)"));
		vo.setBirth(In.getString("생년월일(yyyy-mm-dd)"));
		vo.setTel(In.getString("연락처"));
		vo.setEmail(In.getString("이메일"));
		vo.setPhoto(In.getString("사진"));
