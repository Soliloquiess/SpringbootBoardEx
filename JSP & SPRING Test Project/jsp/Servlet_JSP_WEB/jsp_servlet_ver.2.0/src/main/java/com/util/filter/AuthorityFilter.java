...
public class AuthorityFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		//ServletRequest는 일반 ServletRequest이다. HttpServletRequest가 아님.
		HttpServletRequest req = (HttpServletRequest)request;
		String url = req.getServletPath();
		
		// 권한 처리를 한다.
		// 사용자 권한이 session에 있다.
		//getSession이 없음 여기서 request가 우리가 아는 request가 아니기 떄문
		//req 이걸 HttpServletRequest로 줘야 (위에 설정)
		
		//객체거 어디 포함되어있는지 저장객체에 대해 설명했는데
//		저장객체중 어플리케이션 객체를 세션에 저장하고 세션은 리퀘스트에 집어넣고 리퀘스트는 페이지 컨텍스트에 집어넣는다.
		//페이지 컨텍스트에선 리퀘스트 꺼낼수 있고 리퀘스트에선 세션 꺼낼 수 있다.
