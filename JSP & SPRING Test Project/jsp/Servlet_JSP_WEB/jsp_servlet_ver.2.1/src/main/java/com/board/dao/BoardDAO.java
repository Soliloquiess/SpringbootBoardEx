...
//		System.out.println("BoardDAO.list()");
		...
		try {// 정상처리
			...
			search(pageObject) + " order by no desc ";
...
//		System.out.println("BoardDAO.list().list - " + list);
	...
	private String search(PageObject pageObject) {
		...
		if(pageObject.getWord() != null && !pageObject.getWord().equals("")) {
			condition += " where 1 = 0 ";
			if(pageObject.getKey().indexOf("t") != -1)	//-1이라는 소리는 없다는 뜻
				condition += " or title like ? ";	//키가 있으면 condition에 붙인다.
			//? 나왔으니 데이터 세팅하는 것도 필요함. board데이터에 원본데이터 가져옴. 원본데이터 가져올 떄 데이터 가져옴.
