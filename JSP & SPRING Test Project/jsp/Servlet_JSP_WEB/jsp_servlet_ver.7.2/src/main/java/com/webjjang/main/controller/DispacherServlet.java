...
//			} // if 문 끝
			...
			PrintWriter out = response.getWriter();	//out출력시 클라이언트에 그대로 전달해서 출력
//			if(key.equals("/ajax"))	
			//들어온게 /ajax로 경로가 들어오면
			if(key.equals("/ajax") && (jsp.indexOf("redirect:") != 0))
				out.print(jsp);////out출력시 클라이언트에 그대로 전달해서 출력 count해서 받아온걸 그대로 출력
			else//받아오지 않으면 포워드 시키거나 sendRedirect시킴.
				//ajax받아온 경우에만 순수한 데이터 넘긴다.
