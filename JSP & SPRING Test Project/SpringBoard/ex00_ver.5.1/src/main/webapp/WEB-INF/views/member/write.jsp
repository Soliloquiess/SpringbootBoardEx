	...
	$(function() {
		...
		$("#id").keyup(function() {
			...
			$("#idCheckDiv").load("/member/idCheck?id=" + id, function(result) {
				...
				//위 load에서 갔다왔더니 가능한이 있을 경우
				if (result.indexOf("가능한") == -1) {		//-1이란건 없다는 얘기
					//가능한이 없으면 (-1이면 ) danger하고
					// 중복된 아이디인 경우 배경은 빨간색
					$("#idCheckDiv").addClass("alert-danger");
					idCheck = false;
				} else {
		...
		// 비밀번호 처리 이벤트
		$("#pw").keyup(function() {
			pwCheck = false;	//pwCheck에 false넣고 시작 같을떄만 True로 바꿈
			...
			if (pw == pw2) {
				...
				//pwCheckDiv는 pw1,  pw2CheckDiv 는 pw2 이 두개가 같은지 확인하는 거이므로 2개를 받고 함수 돌린다.
				...
				pwCheck = true;	//아이디와 마찬가지로 가능할 경우 false에서 true로 바꿈
			} else {
		...
		$("#writeForm").submit(function(){
			...
			if(!idCheck){	//idCheck가 true가 아니면
				alert("중복이 되지 않는 적당한 형식의 아이디를 사용하셔야만 합니다.");
				$("#id").focus();	//id 고치도록 focus해줌
				// form 전송을 무시시킨다.
				return false;
			}
			// 비밀번호와 비밀번호 확인
	...
	<div class="container">
		...
		<form action="write.do" method="post" enctype="multipart/form-data" id="writeForm">
			...
			<div class="form-group">
				...
				<div id="pwCheckDiv" class="alert alert-danger">비밀번호는 4자
					이상이여야합니다.</div>
			</div>
			<div class="form-group">
				...
				<div id="pw2CheckDiv" class="alert alert-danger">비밀번호는 4자
					이상이여야합니다.</div>
			</div>
			<div class="form-group">
			...
			<div class="form-group">
				<label for="birth">생년월일</label> <input id="birth" name="birth"
					class="form-control datepicker" required="required" placeholder="yyyy-mm-dd"
					autocomplete="off">
			</div>
			<div class="form-group">
			...
			<div class="form-group">
				<label for="phtoFile">본인사진</label> <input id="photoFile"
